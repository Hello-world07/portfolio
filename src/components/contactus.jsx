...
const ContactUs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://portfoliobackend-2-ok6x.onrender.com/contact", {
        method: 'POST',
        headers: {
      ...
      <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-custom-dark-secondary p-6 sm:p-8 rounded-lg shadow-md glow-effect">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6" noValidate>
            ...
            {status === 'success' && (
              <p className="text-sm sm:text-base text-green-500 dark:text-green-400 font-montserrat text-center" role="status">
                Message sent successfully! I'll get back to you soon.
              </p>
            )}
            {status === 'error' && (
              <p className="mt-1 text-sm text-red-500 dark:text-red-400 font-montserrat text-center" role="alert">
